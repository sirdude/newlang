...
=item syntax.pod  	A Document covering the syntax of our new language

=item notes.txt		Things I've been thinking about

=item src		Current source code for BNFC starting with trying
			to build an lpc grammar and then will modify that
			so for now this doesn't line up with the docs.

=item examples		Example code
