...
basically mathematical bits that evaluate to a value. Expressions consist of
...
call_other and whats the difference, is there a difference?
...
XXX Maybe make the rest of these functions if we still need them?  Similar to 
mod vs % (Maybe make the first two operators).  I really don't like the
confusion of || vs | for new coders.
& vs && and unary - vs -  Make it clear and easy to remember
...
function is defined is sometimes refered to as the scope of the function or
...
nomask type modifer.  You can also use :: to look up the value of a function or
variable at a higher level.  If I have the following code:
	int x;
...
XXX add more info about our examples.
...
There are numerous names for this, branching, conditionals, flow control.
